...
* A frame, $\alpha_{k}$ has two components: its time-step, given by $k$, and the state it encodes, given by its value. Its value is given by $\alpha_{k}=\alpha_{\Omega}(\alpha_{k-1})$.
