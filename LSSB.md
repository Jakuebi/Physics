...
* A perfect simulation, $T$, cannot guess, or make any intermediate steps of what is being simulated, $S$. For each sequential frame of $T$, $(K, X)$, it must be found that there exists $S_{n - m - 1}$ = $T_{n - 1}$ and $S_{n - m}$ = $T_{n}$ OR $S_{n - m}$ = $T_{n}$ and $S_{m - n + 1}$ = $T_{n + 1}$, where $m {\geq} 0$ and $S_{n}$ corresponds to the state of $S$ at respective time-interval $n$ while $T_{n}$ corresponds to $X$ of $(K, X)$ from $T$ when $K = n$. Additionally, a perfect prediction must be a state taken from a perfect simulation. 
